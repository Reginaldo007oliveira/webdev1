  ...
  const addProduct = async (productId: number) => {
    try {
      ...
      const handleNewProductAmount = (amount: number) => {
        ...
        setCart(newProduct);
        localStorage.setItem("@Corebiz:cart", JSON.stringify(newProduct));
      };

      if (!exist) {
        item["amount"] = 1;
        setCart([...cart, item]);
        localStorage.setItem("@CoreBiz:cart", JSON.stringify([...cart, item]));
  ...
  const updateProductAmount = async ({
    ...
    try {
      ...
      const handleNewProductAmount = () => {
        ...
        setCart(newProducts);
        localStorage.setItem("@Corebiz:cart", JSON.stringify(newProducts));
      };
